...
export interface TechDemandGroup {
  key: 'languages' | 'frameworks' | 'databases' | 'cloud' | 'tools';
  label: string;
  items: TechDemand[];
}
...
export interface SearchStats {
  ...
  below_threshold: number; // evaluated jobs that did not reach the threshold (= evaluated - matched)
  ...
  stack_core?: string[];   // candidate languages + frameworks (full stack, for the legend)
  stack_tools?: string[];  // candidate tools
  primary_stack?: string[]; // the candidate's MAIN defining stack — what ranking/matching is based on
  tech_demand?: TechDemand[]; // most-requested technologies across the in-period listings (flat, legacy)
  tech_demand_groups?: TechDemandGroup[]; // same, grouped by type (languages/frameworks/databases/cloud/tools)
